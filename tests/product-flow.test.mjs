...
assert(sidebarIndex > runtimeIndex, "stable sidebar must load after the runtime scheduler");
...
assert.match(index, /2026-08-21-v16\.5-runtime-cleanup/);
...
assert.doesNotMatch(index, /removeAttribute\("data-v2-outline-ready"\)/, "temporary outline inline patch must live in runtime, not index.html");
...
assert.match(runtime, /explicitObserverScheduler/);
assert.match(runtime, /window\.__UNLIMITED_NATIVE_MUTATION_OBSERVER__ = NativeMutationObserver/);
assert.doesNotMatch(runtime, /window\.MutationObserver\s*=/, "V16.5 must leave the browser's global MutationObserver untouched");
...
console.log("V16.5 product contract passed: native browser observers -> explicit scheduler -> stable sidebar -> AI -> manuscript -> data safety.");
