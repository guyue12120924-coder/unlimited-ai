...
4. 原小说产品流契约
5. 陪伴模式 V1/V2/V3/V4/V5 隔离与功能契约

`tests/companion-mode.test.mjs` 会确认陪伴模式不引用小说存储 namespace，确认多角色、搜索/记忆和 V5 恢复层只使用独立 `uai_companion_*` 数据，并验证 Worker 的 Companion Prompt 与长期记忆优先级。
